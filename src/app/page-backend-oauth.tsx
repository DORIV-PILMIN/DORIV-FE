...
/**
 * 백엔드에서 OAuth URL을 생성하는 방식
 * 이 파일을 page.tsx로 복사하여 사용하세요
 */
export default function Home() {
  ...
  const handleOAuthLogin = async (provider: OAuthProvider) => {
    try {
      // localStorage에 provider 저장
      if (typeof window !== "undefined") {
        localStorage.setItem("oauth_provider", provider);
      }

      // 백엔드에서 OAuth URL 가져오기
      const loginUrl = await getOAuthUrlFromBackend(provider);
      window.location.href = loginUrl;
    } catch (error) {
      console.error("OAuth login error:", error);
      setErrorMessage(
        `${provider === "google" ? "Google" : "카카오"} 로그인을 시작할 수 없습니다.`
      );
      setShowError(true);
    }
  };
      ...
      <div className="flex w-full max-w-[900px] h-[550px] bg-white border-4 border-black rounded-lg overflow-hidden shadow-[8px_8px_0px_rgba(0,0,0,1)]">
        ...
        <div className="flex-1 p-10 flex flex-col bg-white">
          ...
          <div className="flex gap-4 mt-auto">
            <a
              href="#"
              className="text-xs text-black underline hover:no-underline cursor-pointer"
            >
              이용약관
            </a>
            <a
        ...
        <div className="flex-1 bg-[#E8E8E8] relative flex items-center justify-center border-l-[3px] border-black">
          ...
          <div className="relative w-[300px] h-[300px] bg-white rounded-full border-[5px] border-black flex items-center justify-center shadow-[6px_6px_0px_rgba(0,0,0,0.3)]">
            <div className="relative w-[260px] h-[260px] flex items-center justify-center">
              ...
              <div
                className="absolute w-[220px] h-[200px] bg-[#D8E5F0] border-[4px] border-black z-[2]"
                style={{
